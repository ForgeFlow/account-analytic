...
def post_load_hook():
    def _new_recompute_payment_terms_lines(self):
        ...
        def _compute_diff_payment_terms_lines(self, existing_terms_lines, account, to_compute):
            ...
            for date_maturity, balance, amount_currency in to_compute:
                ...
                if existing_terms_lines_index < len(existing_terms_lines):
                    ...
                    candidate_vals_update = self.get_candidate_vals_update(date_maturity, amount_currency, balance)
                    candidate.update(candidate_vals_update)
                else:
                    # Create new line.
                    create_method = in_draft_mode and self.env['account.move.line'].new or self.env['account.move.line'].create                    
                    # HOOK 1
                    candidate_vals = self.get_payment_term_candidate_vals(balance, amount_currency, date_maturity, account)
        ...
        # HOOK 2
        self.post_process_term_lines(_get_payment_terms_computation_date ,_get_payment_terms_account, _compute_payment_terms, _compute_diff_payment_terms_lines)        
        # HOOK 2 end
    ...
    AccountMove._recompute_payment_terms_lines = (
        _new_recompute_payment_terms_lines
    )
